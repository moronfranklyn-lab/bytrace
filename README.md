...
应用图标是一枚**朱砂印章**：牛皮纸底 + 朱砂印面 + 纸色阳刻笔画。

选它的理由：这套配色本来就是产品自己的设计语言（三个主题都围绕书卷气），
印章的方框给出"这是一枚印记"的语义，内部刻槽给出"文字"的语义——
合起来正好是产品在做的事：把一个人的风格拆成可辨识的印记。

图标可复现：`python3 scripts/make-icns.py`（需要 Pillow）。
设计过程与取舍记录在该脚本与 `scripts/icon-round2.py` 的注释里。
...
| **安装桌面图标.command** | 单独安装桌面图标（前面的安装步骤没装上、或你想装到别处时用） |
| **启动笔迹.command** | 启动应用。**已装桌面外壳时打开独立窗口**，否则退回浏览器 |

安装后桌面上会出现「笔迹 ByTrace」图标，以后直接双击它就能用。

> **图标没出现？** 双击 `安装桌面图标.command`，它会逐个尝试桌面、用户应用程序目录、
> 系统应用程序目录，并明确报告每个位置的结果。
> 三个位置都写不进去时（macOS 隐私保护会拦），它会给出手动拖拽的完整步骤。
> **不装图标也完全不影响使用** —— 双击 `启动笔迹.command` 一样开应用。
