...
| 候选 | 结论 |
| --- | --- |
| **Electron** | ✅ 采用。生态成熟、调试方便；代价是需额外下载约 130MB 二进制，打包体积偏大 |
| Tauri | ❌ 未采用。产物更小（约 5MB），但需要 Rust 工具链与较长的首次编译，对本项目的收益不足以抵消引入新工具链的成本 |
| 纯浏览器 | 保留为降级路径。未安装 Electron 时启动器自动退回浏览器模式 |

**为什么不把界面直接打进外壳**：后端依赖原生模块（better-sqlite3）与 Node 专属能力
（子进程、文件系统），且页面使用服务端渲染。Electron 渲染进程无法承载这些，
因此保留「本机服务 + 原生窗口」结构，外壳只负责窗口与服务生命周期。
