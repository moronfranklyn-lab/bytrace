...
7. **每节都要服务于核心论点 core_thesis**。core_thesis = ${outline.core_thesis ? `「${outline.core_thesis}」` : '请从开篇 hook 里推断'}。整篇围绕这一句反复挖深，不要每节换个新概念重启。每节还必须给读者新增一种收益：理解、判断、行动、验证或可复用材料。
...
- 第一人称只能用于真实材料、判断和选择责任；不要写未经提供的戏剧化动作、对白、情绪或现场感。
...
- **所有数据必须标明来源**：文章中出现的数字、百分比、统计数据、研究结论、产品参数等，必须在文末或脚注中注明来源。
- **来源标注格式**：使用 Markdown 脚注语法 \`[^1]\`，在数据后添加脚注标记，文末统一列出来源。
- **来源信息要素**：
  - 官方数据：机构名称 + 报告/文档名称 + 发布时间
  - 产品参数：品牌官网 + 产品型号 + 数据获取时间
  - 研究报告：作者/机构 + 报告标题 + 发布年份
  - 新闻报道：媒体名称 + 文章标题 + 发布日期
  - 企业信息：公司名称 + 官方公告/财报 + 时间
- **示例格式**：
  \`\`\`markdown
  峰值扭矩35牛米，整机重量只有450克[^1]

  [^1]: 数据来源：灵足时代官网产品参数，2026年1月
  \`\`\`
- **无法确认来源的数据**：宁可删除，也不要使用。不要编造来源，不要使用"据统计""有研究表明"等模糊表述。
- **素材包中的数据**：如果素材包提供了来源 URL，转换为规范的文字描述作为脚注，不直接放链接。
# 文案温度
